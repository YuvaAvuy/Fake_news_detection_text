...
def predict_with_bert(text):
    ...
    return probs[0][1].item()  # probability of "fake"
...
def predict_with_flan_t5(text):
    # Using prompt-based classification
...
def ensemble_predict(text):
    ...
    avg_fake_prob = (p1 + p2 + p3) / 3
    label = "Fake News" if avg_fake_prob >= 0.5 else "Real News"
    return label, round(avg_fake_prob * 100, 2)
